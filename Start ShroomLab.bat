...
if %errorlevel%==0 (
  ...
  node -e "const h=require('http'),f=require('fs'),p=require('path');h.createServer((q,s)=>{let fp=p.join('.',q.url.split('?')[0]);if(fp==='.'||fp.endsWith('/'))fp=p.join(fp,'index.html');const ext=p.extname(fp);const mime={'html':'text/html','css':'text/css','js':'application/javascript','jpg':'image/jpeg','jpeg':'image/jpeg','png':'image/png','gif':'image/gif','webp':'image/webp','svg':'image/svg+xml','ico':'image/x-icon'};f.readFile(fp,(e,d)=>{if(e){s.writeHead(404);s.end('404 Not Found');}else{s.writeHead(200,{'Content-Type':mime[ext.slice(1)]||'text/plain'});s.end(d);}});}).listen(5500,()=>console.log(' Server running at http://localhost:5500\n Press Ctrl+C to stop\n'));"
  goto :done
)
